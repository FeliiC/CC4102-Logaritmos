...
SplitArbol split(Nodo n, int is_arbolBPlus){
    ...
    Pair par_mediano = n.llaves_valores[(B/2)+1];

    // Armar nodo derecho
    Nodo nodo_der;
    nodo_der.es_interno = n.es_interno;
    nodo_der.k = (B/2);
    guardar_pares(n.llaves_valores, nodo_der.llaves_valores, (B/2), B); // revisar rangos
    ...
    if(is_arbolBPlus==0 && n.es_interno==0){
        guardar_pares(n.llaves_valores, nodo_izq.llaves_valores, 0, B/2);   // revisar rangos
        nodo_izq.k = (B/2);
    } else{
        guardar_pares(n.llaves_valores, nodo_izq.llaves_valores, 0, (B/2 - 1));   // revisar rangos
        nodo_izq.k = (B/2 -1);
    }
    nodo_izq.sgte = n.sgte;
    // guardar hijos solo si es un nodo interno
    if(n.es_interno){
        guardar_hijos(n.hijos, nodo_izq.hijos, 0, (B/2));         // revisar rangos
        guardar_hijos(n.hijos, nodo_izq.hijos, (B/2 + 1), B);   // revisar rangos
    } 
    // Armar resultado
    SplitArbol result;
