...
void insertar_arbolB(Nodo **arbol, int *tamano_arbol, int indice_nodo, int llave, float valor) {
        ...
        if (nodo_lleno(&arbol[indice_hijo][0])) {
            ...
            *(arbol[indice_hijo]) = split_result.nodo_izq;
            ...
            if (llave <= split_result.par_mediano.llave) {
                insertar_arbolB(arbol, tamano_arbol, indice_hijo, llave, valor);
            } else {
                insertar_arbolB(arbol, tamano_arbol, nuevo_indice_der, llave, valor);
            }
...
// funcion para insertar en un arbol B+
void insertar_arbolBPlus(Nodo **arbol, int *tamano_arbol, int indice_nodo, int llave, float valor) {
    ...
    if (nodo_actual->es_interno == 0) {
        ...
        if (nodo_lleno(&arbol[indice_hijo][0])) {
            ...
            *(arbol[indice_hijo]) = split_result.nodo_izq;
            ...
            arbol[indice_nodo][0] = *nodo_actual;
            if (llave <= split_result.par_mediano.llave) {
                insertar_arbolBPlus(arbol, tamano_arbol, indice_hijo, llave, valor);
            } else {
                insertar_arbolBPlus(arbol, tamano_arbol, nuevo_indice_der, llave, valor);
            }
        } else {
            insertar_arbolBPlus(arbol, tamano_arbol, indice_hijo, llave, valor);
        }
...
void insertar_raiz_arbolB(Nodo ***arbol, int *tamano_arbol, int *raiz, int llave, float valor) {
    int indice_raiz = *raiz;
    // si la raiz no esta llena
    if (!nodo_lleno(&(*arbol)[indice_raiz][0])) {
        insertar_arbolB(*arbol, tamano_arbol, indice_raiz, llave, valor);
        ...
        *raiz = nueva_raiz;
        ...
        if (llave <= split_result.par_mediano.llave) {
            insertar_arbolB(*arbol, tamano_arbol, nuevo_indice_izq, llave, valor);
        } else {
            insertar_arbolB(*arbol, tamano_arbol, nuevo_indice_der, llave, valor);
        }
...
void insertar_raiz_arbolBPlus(Nodo ***arbol, int *tamano_arbol, int *raiz, int llave, float valor) {
    // logica analoga a insertar_raiz_arbolB, pero con consideraciones para B+
    int indice_raiz = *raiz;
    if (!nodo_lleno(&(*arbol)[indice_raiz][0])) {
        insertar_arbolBPlus(*arbol, tamano_arbol, indice_raiz, llave, valor);
    } else {
        ...
        *raiz = nueva_raiz;
        if (llave <= split_result.par_mediano.llave) {
            insertar_arbolBPlus(*arbol, tamano_arbol, nuevo_indice_izq, llave, valor);
        } else {
            insertar_arbolBPlus(*arbol, tamano_arbol, nuevo_indice_der, llave, valor);
        }
...
void insertar_en_arbolB(Nodo ***arbol, int *tamano_arbol, int *raiz, int llave, float valor) {
    insertar_raiz_arbolB(arbol, tamano_arbol, raiz, llave, valor);
}
// funcion principal para insertar en árbol B+
void insertar_en_arbolBPlus(Nodo ***arbol, int *tamano_arbol, int *raiz, int llave, float valor) {
    insertar_raiz_arbolBPlus(arbol, tamano_arbol, raiz, llave, valor);
}